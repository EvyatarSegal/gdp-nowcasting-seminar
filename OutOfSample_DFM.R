...
# (Added rm(df) to clear any leftover variables from previous runs)
if (exists("df")) rm(df) 

df <- read_excel("data/clean/combined_monthly_panel_Q_h3.xlsx") %>%
...
if (!exists("target_df")) {
  target_df <- read_excel("data_h3.xlsx", sheet = "target")
}
