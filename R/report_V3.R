...
# --- D. Build the Final DataFrames with Formatting Metrics ---
news_report <- data.frame(
  ...
  mutate(
    # Create the percentage and relative share columns for aesthetic reporting
    Impact_in_Percent = Ensemble_Impact * 100,
    Relative_Share_Pct = (abs(Ensemble_Impact) / sum(abs(Ensemble_Impact), na.rm = TRUE)) * 100
  ) %>%
  arrange(desc(abs(Ensemble_Impact))) # Sorted from highest magnitude to lowest
...
news_print <- news_report %>%
  ...
  select(Variable, Sector, Ensemble_Impact, Impact_in_Percent, Relative_Share_Pct)
...
# Save outputs to CSV so you have the raw formatted data
...
# Custom formatting so Excel renders these visually as percentages but keeps them numeric under the hood
custom_pct_style   <- createStyle(numFmt = "+0.0000\"%\";-0.0000\"%\"")
custom_share_style <- createStyle(numFmt = "0.00\"%\"")
...
# Write the actual numbers (not the strings) so Excel can filter/sort them properly
writeDataTable(wb, "Sector Impact", Sector_report, startRow = 1, startCol = 1, 
               tableStyle = "TableStyleMedium9")

setColWidths(wb, "Sector Impact", cols = 1:4, widths = c(35, 20, 20, 20))

# Apply styles to numeric and percentage columns
addStyle(wb, "Sector Impact", style = float_style, rows = 2:(nrow(Sector_report)+1), cols = 2, gridExpand = TRUE)
addStyle(wb, "Sector Impact", style = custom_pct_style, rows = 2:(nrow(Sector_report)+1), cols = 3, gridExpand = TRUE)
addStyle(wb, "Sector Impact", style = custom_share_style, rows = 2:(nrow(Sector_report)+1), cols = 4, gridExpand = TRUE)

# Apply Conditional Formatting (Traffic lighting) to Impact and Percent columns
conditionalFormatting(wb, "Sector Impact", cols = 2:3, rows = 2:(nrow(Sector_report)+1), rule = ">0", style = pos_style)
conditionalFormatting(wb, "Sector Impact", cols = 2:3, rows = 2:(nrow(Sector_report)+1), rule = "<0", style = neg_style)
...
# Group the numeric attributes together neatly in Excel
news_report_excel <- news_report %>% select(Variable, Sector, Ensemble_Impact, Impact_in_Percent, Relative_Share_Pct)

writeDataTable(wb, "Variable Impact", news_report_excel, startRow = 1, startCol = 1, 
               tableStyle = "TableStyleMedium9")

setColWidths(wb, "Variable Impact", cols = 1:5, widths = c(45, 25, 20, 20, 20))

# Apply styles
addStyle(wb, "Variable Impact", style = float_style, rows = 2:(nrow(news_report_excel)+1), cols = 3, gridExpand = TRUE)
addStyle(wb, "Variable Impact", style = custom_pct_style, rows = 2:(nrow(news_report_excel)+1), cols = 4, gridExpand = TRUE)
addStyle(wb, "Variable Impact", style = custom_share_style, rows = 2:(nrow(news_report_excel)+1), cols = 5, gridExpand = TRUE)

conditionalFormatting(wb, "Variable Impact", cols = 3:4, rows = 2:(nrow(news_report_excel)+1), rule = ">0", style = pos_style)
conditionalFormatting(wb, "Variable Impact", cols = 3:4, rows = 2:(nrow(news_report_excel)+1), rule = "<0", style = neg_style)
